...
curl -fsS -X POST https://jingxiangguo.com/api/dashboard/project-update \
  -H "content-type: application/json" \
  -H "x-dashboard-token: $DASHBOARD_WRITE_TOKEN" \
...
For durable project-card framing, use `POST /api/dashboard/project-update` with
