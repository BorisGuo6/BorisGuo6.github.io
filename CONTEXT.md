...
- **Vercel Blob snapshot**: A persisted JSON snapshot of the dashboard state in Vercel Blob. It replaces Supabase as the first Vercel-hosted mutable backend.
...
- The public read surface must continue to work from static JSON; the hosted backend is an interactive state mirror, not the only source of truth.
